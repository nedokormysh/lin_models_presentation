...
from model import load_data, load_model, class_report, top_features, single_prediction
...
def side_bar_params():
    ...
    width_pie = st.sidebar.slider("plot pie width", 1, 10, 3)
    height_pie = st.sidebar.slider("plot pie height", 1, 10, 3)

    st.sidebar.markdown("---")

    st.sidebar.markdown("*Настройки отображения графиков категориальных признаков*")

    width_cat = st.sidebar.slider("plot cat width", 1, 20, 6)
    height_cat = st.sidebar.slider("plot cat height", 1, 10, 3)

    return width_cont, height_cont, width_pie, height_pie, width_cat, height_cat


def process_main_page():

    global analysis_cont, analysis_cat, labels, title
    show_main_page()

    with st.spinner('Подгрузка данных...'):
        datasets, df_dict, datasets_names = open_data()
    st.success('Данные загружены!')

    with st.spinner('Ещё немного. Загрузка обученной модели...'):
        model = load_model()
    st.success('Обученная модель загружена!')
    ...
    page = st.sidebar.selectbox("Выбрать страницу",
                                ["Анализ датасетов",
                                 "Предсказание модели"])
    st.markdown("___")
    st.markdown('*Слева выберете страницу отображения! Анализ датасета или предсказание модели.*')
    st.markdown("___")

    if page == 'Анализ датасетов':
        ...
        st.pyplot(corr_matrix(all_data), use_container_width=False)
        ...
        width_cont, height_cont, width_pie, height_pie, width_cat, height_cat = side_bar_params()
        ...
        st.write('По условию требовалось построить графики распределений и зависимостей целевой переменной и признаков.\n'
                 ...
                 'Для отрисовки графика необходимо выбрать признак.'# А затем нажать кнопку: отобразить график.'
                 )

        st.write('*Размер графика можно поменять в панели слева.*')
        ...
        #     st.write('Вы выбрали:', cont_feat)
        #     st.text(analysis_cont)
        #     st.pyplot(show_continuous_grafics(all_data, cont_feat, width_cont, height_cont), use_container_width=True)
        st.write('Вы выбрали:', cont_feat)
        st.text(analysis_cont)
        st.pyplot(show_continuous_grafics(all_data, cont_feat, width_cont, height_cont), use_container_width=True)
        ...
        if cat_feat == 'LOAN_NOW':
            ...
            analysis_cat = ('Сложно судить о наличии влияния данного признака на целевую переменную.'
                            ' Мало объектов представлено в неосновных категориях')
        elif cat_feat == 'TERM':
        ...
        #     st.write('Вы выбрали:', cat_feat)
        #     st.text(analysis_cat)
        #     if pie:
        #         st.pyplot(pie_categorical_grafics(all_data, cat_feat, labels, title, width_pie, height_pie),
        ...
        st.pyplot(target_grafics(all_data, width_pie, height_pie), use_container_width=False)
        ...
        st.markdown('# Модель линейной классификации')
        st.text('Будем прогнозировать отлик клиентов на рекламные предложения от банка при помощи логистической регресии')

        X_train, X_test, y_train, y_test = load_data(data=all_data)

        st.markdown('## Топ-6 признаков с наибольшими по модулю весами модели')
        df_top_features = top_features(model)
        st.table(df_top_features)

        st.text('В моей реализации модели получил среди топ-6 только категориальные признаки')

        st.markdown('## Зависимость метрик от выбора порога')
        st.text('Наши метрики зависят от выбранного порога. Зададим его заранее')

        th = st.slider('Выберите порог предсказания',  min_value=0.0, max_value=1.0, value=0.5, step=0.01)

        recall, precision, f1 = class_report(model, X_test, y_test, th)
        st.markdown(f'Выбранный порог: {th}. При данном пороге наши метрики: '
                 f'**recall** = {round(recall, 7)}, **precision** = {round(precision, 7)}, **f1** = {round(f1, 7)}')


        st.write('По условиям задачи наша метрика - полнота')

        st.markdown('## Прогноз модели для конкретного клиента')

        st.write(f'Выберем клиента из тестовой выборки. Всего у нас в тестовой выборке {len(X_test)} клиентов')
        id_client = st.selectbox('Index', (range(0, len(X_test))))

        st.write(f'Клиент с id = {id_client} в тестовом датасете имеет следуещее описание')

        st.write(pd.DataFrame(X_test.iloc[id_client, :]).transpose())

        prob, prediction = single_prediction(model, X_test, id=id_client, threshold=th)

        st.write(f'При пороге {th} предсказание для выбранного клиента = {prediction}, '
                 f'т.к. модель прогнозирует вероятность, что отклик был зарегистрирован = {round(prob[0], 6)}')
