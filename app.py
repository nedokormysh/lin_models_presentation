...
def process_main_page():
    ...
    if st.button('Отобразить график выбранного категориального признака'):
        ...
        if pie:
            st.pyplot(pie_categorical_grafics(all_data, cat_feat, labels, title, width_pie, height_pie), use_container_width=True)

        st.pyplot(show_categorical_grafics(all_data, cat_feat, width_cat, height_cat), use_container_width=True)
